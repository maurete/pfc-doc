...
generar (iteraciones), y $p$ es la proporción de elementos de
entrenamiento a utilizar en cada partición de validación. Por defecto,
se utilizan los valores $k=10$ y $p=1/k$.

Cada una de las $k$ particiones de validación se compone con ejemplos
del conjunto de entrenamiento $D^E$, tomando $p\cdot\ell^E$ elementos
en secuencia, contando a partir del elemento $j\frac{\ell^E}{k}$.  La
$j$-ésima partición de validación, $j=1,\ldots,k$, viene dada por
%
\begin{align}
...
\end{align}
%
Cuando $u>\ell^E$, se utiliza el índice $\u^*=u-\ell^E$, por lo que $u$
funciona de modo ``circular''.

Los conjuntos de estimación correspondientes contienen los restantes
